...
.defines(function() {
    ig.SplashLoader = ig.Loader.extend({
        ...
        draw: function() {
            ...
            var isPortrait = this.gw < this.gh;

            var bgPortraitCache = imgCache['bg-01']
            var bgLandscapeCache = imgCache['bg-02']
            if(!bgPortraitCache || !bgLandscapeCache) return;

            context.save()
            if(isPortrait){
                var bgImage = ig.Image.cache[bgPortraitCache.path.img]
            } else {
                var bgImage = ig.Image.cache[bgLandscapeCache.path.img]
            }

            var scaleX = this.gw / bgImage.width;
            var scaleY = this.gh / bgImage.height;
            var bgScale = Math.max(scaleX, scaleY)

            context.translate(this.centerX, this.centerY)
            context.scale(bgScale, bgScale)
            var bgX = -(bgImage.width/2)
            var bgY = -(bgImage.height/2)
