...
.defines(function(){
	EntityStageCtrl = EntityPageController.extend({
		...
		create:function(){
			...
			this.closeBtn = ig.CallAsset.addFrame(this.stageBg.x + (this.stageBg.width * 0.5), -this.stageBg.size.y * 0.5, 'ingame/buttons/close', {}, EntityClickBtn);
			...
			this.showTitle = ig.game.addText(0, this.closeBtn.y + 5, _STRINGS['Game']['stage'], Math.floor(this.closeBtn.height * 0.8), fonts.font1);
			...
			this.gTags = ig.game.addGroup(this.centerX, this.nextBtn.y)
			this.gInGame.add(this.gTags)
			...
			for(var a = 0; a < countStage; a++){
				var tag = ig.CallAsset.addFrameImage(0, 0, 'ingame/stages/page_tag2');
				tag.anchor.setTo(0.5);
				this.gTags.add(tag);

				var mid = parseInt(countStage/2);
				var border = 1.2;
				var x = - (tag.width * border * mid);
				if(countStage % 2 == 0) x = - (tag.width * (border/2)) - (tag.width * border * (mid - 1));
		...
		updateOnOrientationChange:function(){
			...
			this.prevBtn.y = (this.gh * 0.9) + diffY;
			this.nextBtn.y = this.prevBtn.y;
			this.gTags.y = this.nextBtn.y
		},
