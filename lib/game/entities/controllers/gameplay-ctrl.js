...
.defines(function(){
	EntityGameplayCtrl = EntityPageController.extend({
		...
		create:function(){
			...
			this.gList = ig.game.addGroup(0, this.gh * 0.225, {itemLists:this.itemLists}, EntityItemList);
		...
		lastCall:function(){
			...
			var board = this.gBoard.gFrontBoard;
			var boardBound = board.getBounds();
			var boardDist = ig.game.math.distance(board.pos.x, board.pos.y, sourceBound.centerX, sourceBound.centerY)
			var boardRad = ig.game.math.angleBetween(board.pos.x, board.pos.y, sourceBound.centerX, sourceBound.centerY)
			var localX = board.x + (Math.cos(boardRad) * boardDist)
			var localY = board.y + (Math.sin(boardRad) * boardDist)
