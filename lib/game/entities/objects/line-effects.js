...
.defines(function(){
	EntityLineEffects = EntityGroup.extend({
		...
		settingDoughnutCircle:function(){
			this.outerCircle = ig.CallAsset.addFrame(0, 0, 'effects/match01');
			var anchorX = 0.5
			var anchoredWidth = this.outerCircle.width * anchorX;
			var anchorY = anchoredWidth / this.outerCircle.height
			this.outerCircle.anchor.setTo(anchorX, anchorY);
			this.outerCircle.scale.setTo(0)
			this.add(this.outerCircle);

			var frames = ig.GenerateFrameNames('effects/match', 1, 20, '', 2);
		...
		settingLineEffect:function(){
			...
			for(var a = 0; a < totEffect; a++){
				var effect = ig.CallAsset.addFrame(0, 0, 'effects/1')
				effect.anchor.setTo(0.25, 0.5);

				var frames = ig.GenerateFrameNames('effects/', 1, 6, '', 1);
			...
			for(var a = 0; a < this.effects.length; a++){
				...
				if(effect.targets.length > 0){
					...
					if(target){
						...
						// var localPos = effect.groupParent.toLocal(targetBound.centerX, targetBound.centerY)
					}
				}
				...
				if(effect.goTo == 'left'){
					...
					if(width > effect.width) width = effect.width;
				...
				// if(width >= 1){
				// 	effect.visible = true;
				// 	effect.animSheet.drawSize.x = width;
				// } else {
