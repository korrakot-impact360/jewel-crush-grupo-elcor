	...
	EntityPiece = EntitySprite.extend({
		...
		checkIfMatchGoal:function(){
			...
			if(exists && exists.itemLeft > 0){
				...
				// var globalX = exists.pos.x + (exists.size.x * exists.anchor.x);
				// var globalY = exists.pos.y + (exists.size.y * exists.anchor.y);
				// var local = this.groupParent.toLocal(globalX, globalY);
				var globalPos = this.groupParent.toGlobal(this.x, this.y);
				var localPos = exists.groupParent.toLocal(globalPos.x, globalPos.y)
				exists.itemLeft--;
				this.x = localPos.x;
				this.y = localPos.y;
				this.groupParent.remove(this);
				exists.groupParent.add(this);
				this.goToGoal(exists.x, exists.y);
			} else {
				this.kill();
			}
		},
