...
.defines(function(){
	EntityPiece = EntityManualDrawEntity.extend({
		row:-1,
		col:-1,
		_row : -1,
		_col : -1,
		...
		init:function(x, y, settings){
			...
			this.setShape(tileData.sourceSize.w, tileData.sourceSize.h)

			this.pieceData = ig.CallAsset.searchSpriteData('game/gem' + this.pieceId)
			this.pieceImage = ig.Image.cache[imgCache['game'].path.img]
		...
		drawShape:function(){
			...
			context.save()
			var imgW = this.pieceData.frame.w;
			var imgH = this.pieceData.frame.h;
			var imgSX = this.pieceData.frame.x;
			var imgSY = this.pieceData.frame.y;
			var imgX = (this.width - imgW)/2
			var imgY = (this.height - imgH)/2;
			this.pieceImage.draw(imgX, imgY, imgSX, imgSY, imgW, imgH)
		...
		createSparkling:function(){
			...
			var rndX = ig.game.rnd.realInRange(-20, 20);
			var rndY = ig.game.rnd.realInRange(-(this.height * 0.3), -(this.height * 0.2));
		...
		fusePieces:function(pieces, typePower){
			...
			for(var a = 0; a < pieces.length; a++){
				...
				if(piece == this) {
					var checkPiece = curState().gBoard.pieces[this.row][this.col];
					if(checkPiece == this){
						var effect = ig.CallAsset.addFrame(this.x, this.y, 'effects/match09');						
						var anchorX = 0.5
