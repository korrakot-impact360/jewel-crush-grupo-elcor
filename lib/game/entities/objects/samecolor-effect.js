	...
	EntitySamecolorEffect = EntityGroup.extend({
		...
		createEffects:function(){
			for(var a = 0; a < this.targets.length; a++){
				...
				var effect = ig.CallAsset.addFrame(0, 0, 'effects/match01');
				...
				var frames = ig.GenerateFrameNames('effects/match', 6, 8, '', 2);
				effect.addAnim('run', 0.02, frames, true);
				effect.playAnim('run')
				effect.rotation = rad;
				effect.onGoing = true;
		...
		moveEffects:function(){
			...
			for(var a = 0; a < this.effects.length; a++){
				...
				var targetBound = target.getBounds()
				var targetDist = ig.game.math.distance(this.x, this.y, target.x, target.y)
				var targetRad = ig.game.math.angleBetween(this.x, this.y, target.x, target.y)
				...
				var speed = targetDist / this.speed;
				speed = speed * ig.system.tick * ig.GameData.fps

				var speedX = speed * Math.cos(targetRad);
				var speedY = speed * Math.sin(targetRad);
