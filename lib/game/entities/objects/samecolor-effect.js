	...
	EntitySamecolorEffect = EntityGroup.extend({
		...
		moveEffects:function(){
			for(var a = 0; a < this.effects.length; a++){
				...
				var tile = curState().gBoard.tiles[target.row][target.col];
				var tileGlobal = curState().gBoard.gPieces.toGlobal(tile.x, tile.y);
				// var targetBound = target.getBounds();
				var targetBound = ig.game.geom.rectangle(tileGlobal.x, tileGlobal.y, tile.size.x, tile.size.y)
				var localTarget = this.toLocal(tileGlobal.x, tileGlobal.y);
