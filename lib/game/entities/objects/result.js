...
.defines(function(){
	EntityResult = EntityGroup.extend({
		...
		create:function(){
			...
			this.ribbon = ig.CallAsset.addFrameImage(0, -this.bg.size.y * 0.43, 'game/result-ribbon')
			...
			this.scoreTxt = ig.game.addText(-(this.bg.size.x * 0.3), -this.bg.size.y * 0.14, _STRINGS['Game']['score'], 70, fonts.font1);
			...
			this.goalBox = ig.CallAsset.addFrameImage(0, this.bg.size.y * 0.12, 'game/result-list');
			this.goalBox.anchor.setTo(0.5);
			this.add(this.goalBox);

			this.homeBtn = ig.CallAsset.addFrame(-this.bg.size.x * 0.2, this.bg.size.y * 0.31, 'ingame/buttons/menu', {}, EntityClickBtn);
