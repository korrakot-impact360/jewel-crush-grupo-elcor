...
.defines(function(){
	EntityResult = EntityGroup.extend({
		...
		create:function(){
			this.isIn = false;
			this.bg = ig.CallAsset.addFrameImage(0, 0, 'game/result-bg');
			...
			this.scoreTxt = ig.game.addText(-(this.bg.size.x * 0.35), -this.bg.size.y * 0.13, _STRINGS['Game']['score'], 35, fonts.font1);
			this.scoreTxt.anchor.setTo(0, 0.5);
			this.scoreTxt.fill = '#cc7509';
			this.add(this.scoreTxt)

			this.showScore = ig.game.addText(this.bg.size.x * 0.35, this.scoreTxt.y, '0000', this.scoreTxt.fontSize, fonts.font1);
		...
		createList:function(){
			...
			for(var a = 0; a < goalLists.length; a++){
				...
				var half = goalLists.length / 2;
				var border = 1.2;
				var intHalf = parseInt(half);
				if(goalLists.length % 2 == 0) intHalf -= (border / 2);
				var startX = -(item.width * item.scale.x * intHalf * border);
				item.x = startX + (item.width * item.scale.x * a * border)

				var sign = ig.CallAsset.addFrameImage(item.x + (item.width * item.scale.x * 0.5), item.y + (item.height * item.scale.y * 0.5), 'game/result-uncomplete');
				sign.anchor.setTo(1);
				// list.itemLeft = 0
				if(list.itemLeft == 0){
		...
		update:function(){
			...
			var border = 135;
			if(this.showScore.size.x >= border && this.showScore.scale.x == 1){
				this.showScore.scale.setTo(border/this.showScore.size.x);
			}

			if(this.showHigh.size.x >= border && this.showHigh.scale.x == 1){
				this.showHigh.scale.setTo(border/this.showHigh.size.x)
			}
		}
	})
})