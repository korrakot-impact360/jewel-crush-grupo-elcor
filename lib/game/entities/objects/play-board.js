...
.defines(function(){
	EntityPlayBoard = EntityGroup.extend({
		...
		createMatchEffect:function(piece){
			var spriteName = 'game/effects/match01';
			var seq = ig.GenerateFrameNames('game/effects/match', 1, 20, '', 2)
			var timer = 0.02;
			if(piece.powerUpId == 5) {
				spriteName = 'game/effects/bomb1';
				seq = ig.GenerateFrameNames('game/effects/bomb', 1, 9, '', 1)
				timer = 0.03;
			}
			...
			anim.onEnterFrame.add(function(){
				// if(this.tile == 'game/effects/match09'){
				if(this.spriteParent.piece.powerUpId < 5 && this.frame == 8){
		...
		createPiece:function(row, col, powerUpId, pieceId){
			...
			var frameName = 'game/gem' + id;

			if(powerUpId > 0){
				className = EntityPowerUp
				if(powerUpId > 3){
					frameName = 'game/powerup' + powerUpId;
				} else {
					frameName = 'game/effects/line/gem' + id + '-' + powerUpId
				}
			} else {
				if(posPowerUp > 95 && !curState().gList.isCompleted && curState().pageReady){
					...
					if(powerUpId > 3){
						frameName = 'game/powerup' + powerUpId;
					} else if(powerUpId > 0 && powerUpId <= 3){
						frameName = 'game/effects/line/gem' + id + '-' + powerUpId
					}
