...
.defines(function(){
	EntityItemList = EntityGroup.extend({
		...
		create:function(){
			...
			var itemBox = {
				x : (listBoxData.frame.w * 0.5) - (itemBoxData.frame.w * 0.52),
				y : -4
			}
			...
			for(var a = 0; a < this.itemLists.length; a++){
				...
				var icon = ig.CallAsset.addFrameImage(0, itemBox.y, 'game/gem' + id);
				icon.anchor.setTo(0.5);
				icon.scale.setTo(0.5)
				icon.jewelId = id;
				this.add(icon);

				icon.x = itemBox.x - ((itemBoxData.frame.w / 5.5) * 2) + ((itemBoxData.frame.w / 5.5) * a);
